...
coeff_df = pd.DataFrame([Linear_reg.coef_],X.columns)
coeff_df
...
Accuaracy_LinearRegression=Linear_reg.score(x_train,y_train)
...
pd.DataFrame(y_pred,columns=["Predicted_Value"])
...
Accuaracy_LassoRegression=Lasso_reg.score(x_train,y_train)
print("Accuracy of Lasso Regression Model is",format(Accuaracy_LassoRegression))
...
Accuaracy_RidgeRegression=Ridge_reg.score(x_train,y_train)
...
y_pred = polynomial_reg.predict(poly_reg.transform(x_test))
pd.DataFrame(y_pred,columns=["Predicted_Value"])
...
Accuaracy_PolynomialRegression=polynomial_reg.score(X_poly, Y)
